...
export async function scrapeAmazonProduct(url) {
  ...
  try {
    ...
    // Calculate lowest, highest, and average price correctly
    const priceList = [
      { price: currentPrice || 0, type: "currentPrice" },
      { price: originalPrice || 0, type: "originalPrice" },
    ];

    const lowestPrice = getLowestPrice(priceList);
    const highestPrice = getHighestPrice(priceList);
    const averagePrice = getAveragePrice(priceList);
    ...
    console.log(
      "Response during Scraping & encoding details of the product => ",
      data
    );
    return data;
  } catch (err) {
